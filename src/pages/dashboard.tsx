...
import { getPatientSymptomReports } from 'apiSdk/patient-symptom-reports';
import { PatientSymptomReportInterface } from 'interfaces/patient-symptom-report';
import { getHpcMeetingEvaluations } from 'apiSdk/hpc-meeting-evaluations';
import { HpcMeetingEvaluationsInterface } from 'interfaces/hpc-meeting-evaluations';
import { getHpcMeetingMembers } from 'apiSdk/hpc-meeting-members';
import { HpcMeetingMembersInterface } from 'interfaces/hpc-meeting-members';

import { PatientSubstanceUseListPage as ShowCaseList } from 'pages/patient-substance-uses';

function DashboardPage() {
  const { session } = useSession();

  const patientSymptomReportStat = useSWR<PaginatedInterface<PatientSymptomReportInterface>>(
    () => `/patient-symptom-reports`,
    () => getPatientSymptomReports({ limit: 1, offset: 0 }),
  );

  const hpcMeetingEvaluationsStat = useSWR<PaginatedInterface<HpcMeetingEvaluationsInterface>>(
    () => `/hpc-meeting-evaluations`,
    () => getHpcMeetingEvaluations({ limit: 1, offset: 0 }),
  );

  const hpcMeetingMembersStat = useSWR<PaginatedInterface<HpcMeetingMembersInterface>>(
    () => `/hpc-meeting-members`,
    () => getHpcMeetingMembers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Patient Symptom Reports',
      result: patientSymptomReportStat?.data?.totalCount,
      isLoading: patientSymptomReportStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Hpc Meeting Evaluations',
      result: hpcMeetingEvaluationsStat?.data?.totalCount,
      isLoading: hpcMeetingEvaluationsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Hpc Meeting Members',
      result: hpcMeetingMembersStat?.data?.totalCount,
      isLoading: hpcMeetingMembersStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Patient Substance Uses
                </Text>
                <NextLink href={`patient-substance-uses`} passHref legacyBehavior>
                  <Button
                  ...
                  >
                    See all Patient Substance Uses
                  </Button>
                </NextLink>
              </Flex>
