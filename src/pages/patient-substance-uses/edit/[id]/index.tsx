...
import { getPatientSubstanceUseById, updatePatientSubstanceUseById } from 'apiSdk/patient-substance-uses';
...
function PatientSubstanceUseEditPage() {
  ...
  const handleSubmit = async (values: PatientSubstanceUseInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePatientSubstanceUseById(id, values);
      mutate(updated);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Patient Substance Use',
              isCurrent: true,
            },
          ]}
        />
