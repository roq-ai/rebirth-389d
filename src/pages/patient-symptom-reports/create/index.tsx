...
import { createPatientSymptomReport } from 'apiSdk/patient-symptom-reports';
...
function PatientSymptomReportCreatePage() {
  ...
  const handleSubmit = async (values: PatientSymptomReportInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createPatientSymptomReport(values);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Create Patient Symptom Report',
              isCurrent: true,
            },
          ]}
        />
...
)(PatientSymptomReportCreatePage);
